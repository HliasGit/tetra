...
__global__ void compute_apex_float4(   dim_t *grid, cube_gpu_SoA *d_relevant_cubes, int number_relevant_cubes,
    ...
    for(int v=0; v<8; v++){
        d_cube_points_coordinates[tid * 8 + 7].val.w = grid[(int)(d_cube_points_coordinates[tid * 8 + 7].val.z +
                                                            d_cube_points_coordinates[tid * 8 + 7].val.y * dim->z_dim +
                                                            d_cube_points_coordinates[tid * 8 + 7].val.x * dim->z_dim * dim->y_dim)];
    }
}
